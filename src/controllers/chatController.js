...
async function getDetail(req, res) {
    ...
    try {
        ...
        const response = { id: panditId, name: order?.display_name, status: order?.status, profile: order?.profile, isOnline: order?.chat, is_free: orderDetail?.is_free, pandit_id: orderDetail?.pandit_id }
        if (orderDetail) {
...
async function endChat(req, res) {
    ...
    try {
        ...
        if ((diffMinutes < Number(setting?.chat_end_min_minutes)) && !order?.is_free) return res.status(400).json({ success: false, message: "Can't end chat in first 1 minute." });
