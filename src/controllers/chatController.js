...
async function endChat(req, res) {    const { orderId } = req.body;    if (!orderId) {        return res.status(400).json({ success: false, message: 'Missing params.' });    }    try {        const order = await db('orders').where({ userId: req.userId, orderId }).first();        if (!order) {            return res.status(400).json({ success: false, message: 'Wrong order. Please enter correct' });        }        const [{ total }] = await db('orders').where({ panditId: order?.panditId, userId: req.userId }).count('id as total');        if (total == 1) {            return res.status(400).json({ success: false, message: 'You can not end this chat.' });        }        if (order.status != 'continue') {            return res.status(400).json({ success: false, message: 'order is pending or completed.' });        }        await db('orders').where({ orderId }).update({ status: "completed", endTime: new Date() });        return res.status(200).json({ success: true, data: null, message: 'End chat successfully.' });    } catch (err) {        console.error(err);        res.status(500).json({ success: false, message: 'Server error' });    }}module.exports = { getRoom, getMessage, sendMessage, getDetail, getOrderDetail, endChat };